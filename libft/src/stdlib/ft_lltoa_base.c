...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/18 00:55:33 by stanislav         #+#    #+#             */
/*   Updated: 2022/04/18 00:55:34 by stanislav        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	ft_get_numlen(LL nbr, int nbase)
{
	size_t	len;
...
static char	*ft_init_lltoa(LL nbr, const char *base, int *sign, int *nbase)
{
	char	*str;
	size_t	nbrlen;
...
char	*ft_lltoa_base(LL nbr, const char *base)
{
	char	*str;
	int		pos;
	int		nbase;
	...
	str = ft_init_lltoa(nbr, base, &sign, &nbase);
	if (!str)
		return (NULL);
	pos = 0;
	if (nbr < 0)
